...
for a in angles:
    ...
    diff_dists.append(d1 - d2)
    sum_dists.append(d1 + d2)

# --- Setup Animation with 4 Subplots ---
...
trace_line1, = ax1.plot([], [], 'r-', linewidth=1, alpha=0.5)
trace_line2, = ax1.plot([], [], 'm-', linewidth=1, alpha=0.5)
circle1 = plt.Circle((0, 0), r, color='green', alpha=0.4)
circle2 = plt.Circle((0, 0), r, color='orange', alpha=0.4)
ax1.add_artist(circle1)
ax1.add_artist(circle2)
ray1, = ax1.plot([], [], 'g--', alpha=0.4)
ray2, = ax1.plot([], [], 'orange', linestyle='--', alpha=0.4)
...
ax1.set_title('Opposing Circles')
...
# Subplot 3: Difference (d1 - d2)
diff_plot, = ax3.plot([], [], 'purple', linewidth=2, label='d1 - d2')
ax3.set_xlim(0, 360)
ax3.set_ylim(min(diff_dists) - 1, max(diff_dists) + 1)
ax3.set_xlabel('Angle (deg)')
ax3.set_ylabel('Difference')
ax3.set_title('Difference (d1 - d2)')
ax3.grid(True, linestyle='--', alpha=0.6)
ax3.legend(fontsize='x-small')

# Subplot 4: Sum (d1 + d2)
sum_plot, = ax4.plot([], [], 'b-', linewidth=2, label='d1 + d2')
ax4.set_xlim(0, 360)
ax4.set_ylim(min(sum_dists) - 1, max(sum_dists) + 1)
ax4.set_xlabel('Angle (deg)')
ax4.set_ylabel('Total Distance')
ax4.set_title('Sum (d1 + d2)')
...
def init():
    ...
    diff_plot.set_data([], [])
    sum_plot.set_data([], [])
    return trace_line1, trace_line2, circle1, circle2, ray1, ray2, line_d1, line_d2, diff_plot, sum_plot
...
def update(frame):
    ...
    # Update Subplot 3
    diff_plot.set_data(angles[:frame+1], diff_dists[:frame+1])
